...
const date = new Date();
const day_string = (date.getDay() + 1).toString().padStart(2, '0');
        ...
        if (!gameTime || !awayTeam || !homeTeam) {
            ...
            req.end(function (res) {
                ...
                res.body.data.forEach(game => {
                    let start_time = timeConverter(parseInt(game.status.substring(0, 1))) + game.status.substring(1, 4);
                    gameTime = start_time;
            ...
            client.channels.get('566703016443510798').send("Today's game: " + awayTeam + " @  " + homeTeam + "\nGame starts at " + gameTime);
        }
    }
});
